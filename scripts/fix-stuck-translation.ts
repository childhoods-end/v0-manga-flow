...
async function fixStuckTranslation(projectId: string) {
  ...
  pages?.forEach((page: any) => {
    ...
    console.log(`  Page ${page.page_index + 1}: ${hasProcessed ? '✅' : '❌'} ${hasError ? `⚠️ Error: ${hasError}` : ''}`)
    ...
    if (hasError) {
      console.log(`    Error details:`, {
        page_error: metadata.page_error,
        ocr_error: metadata.ocr_error,
        render_error: metadata.render_error,
        upload_error: metadata.upload_error,
      })
    }
  })
...
if (!projectId) {
  console.error('Usage: tsx scripts/fix-stuck-translation.ts <projectId>')
  process.exit(1)
}
